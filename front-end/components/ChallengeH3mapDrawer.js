...
function ChallengeH3mapDrawer({ mapRef }) {
    const [sortedData, setSortedData] = useState([]);
    const [isLoading, setIsLoading] = useState(false);
    const [page, setPage] = useState(1);
    const pageSize = 100; // Set page size for infinite scrolling
    const distinctMarkerRef = useRef(null);
    const [sortType, setSortType] = useState({ field: 'challenge_counts', order: 'desc' });

    // Load initial data
    useEffect(() => {
        const dataFile = `challenge_sorted_by_${sortType.field}_${sortType.order}.json`;
    ...
    return (
        <Drawer variant="permanent" anchor="right" open={true}>
            <Select
                ...
                fullWidth
            >
                <MenuItem value="challenge_counts">Sort By: Number of Challenges</MenuItem>
                <MenuItem value="pct_failed_challenges">Sort By: Success Rate</MenuItem>
                ...
                {sortedData.map((item, index) => (
                    <ListItem key={index}>
                        <div>
                            ...
                            <div>Number of Challenges: {item.challenge_counts}</div>
                            <div>Success Rate: {item.pct_failed_challenges.toFixed(2)}</div>
...
export default ChallengeH3mapDrawer;
