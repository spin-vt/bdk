...
def load_loss_to_color_mapping(lcf_file_path, floor_loss_rate=None):
    ...
    with open(lcf_file_path) as file:
        for line in file:
    ...
    with Image.open(input_image_path) as img:
        ...
        for i in range(img.width):
            for j in range(img.height):
                ...
                loss_rate = next(
                    (loss for loss, color in loss_to_color_mapping.items() if color == (r, g, b)),
                    None,
                )
    ...
    try:
        with zipfile.ZipFile(kmz_filename, "r") as kmz:
            with kmz.open("doc.kml", "r") as kml_file:
                ...
                return {"nbound": north, "sbound": south, "ebound": east, "wbound": west}
